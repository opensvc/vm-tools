...
variable "vm_template_name" {
  type    = string
  default = "packer-uefi-u2604.qcow2"
}
...
source "qemu" "custom_image" {
  ...
  http_directory = "http"
  #iso_url   = "https://ubuntu.mirrors.ovh.net/ubuntu-releases/26.04/${var.ubuntu_iso_file}"
  iso_url   = "../images/${var.ubuntu_iso_file}"
  iso_checksum = "file:https://last-public-canonical-ubuntu-releases.snap.mirrors.ovh.net/26.04/SHA256SUMS"
  memory = 4096
...
build {
  ...
  provisioner "shell" {
    execute_command = "echo 'opensvcpacker' | sudo -S env {{ .Vars }} bash '{{ .Path }}'"
